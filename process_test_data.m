...
load(['Data/human_robot_data_', name_str,'_test.mat']);
load(['Data/robot_data_10Hz_', name_str,'_test.mat']);
...
if (check_offset)
    ...
    save(['Data/Inference/Means/k_delay_test_',name_str,'.mat'], 'k_delay');
    keyboard;
end
...
numExps = (nn - ss*K)/K-2; % total number of experiments
L = 4; % number of disturbances
n = size(x_expert,1);
m = size(u_expert,1);
par = [n, m, L, N, K];
% define the structure of disturbances
