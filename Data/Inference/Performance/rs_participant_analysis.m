...
figure()
x_rel_actual = x_expert(6,:) - x_expert(1,:);
ttt = 560:610; ttt = 0.1*ttt;
plot(ttt, (x_rel_actual(560:610)-2.5)/l_car, 'g', 'LineWidth', 5.);
hold on;
for k=39:40
    t_k = (k-1)*K+1:k*K+1;
    t_k = 0.1*t_k;
    plot(t_k, (x_rel(1,:,k)-2.55)/l_car, 'r', 'LineWidth', 5, 'MarkerSize', 3);
    hold on;
    plot(t_k, (x_rel_n(1,:,k)-2.5)/l_car, 'b', 'LineWidth', 5, 'MarkerSize', 3);
    plot(0.1*(K*(k-1)+1)*ones(1,21), 0.5:0.1:2.5, 'k', 'LineWidth', 4)
    plot(0.1*(K*(k-1)+9)*ones(1,21), 0.5:0.1:2.5, 'k:', 'LineWidth', 4)
    plot(0.1*(K*k+1)*ones(1,21), 0.5:0.1:2.5, 'k', 'LineWidth', 4)
    annotation('doublearrow', [0.31 0.41], [0.25,0.25], 'LineWidth', 3);
    text(57.8,0.4,['Prepare'],'HorizontalAlignment','right', 'Fontsize', 25);
    annotation('doublearrow', [0.44 0.52], [0.25 0.25], 'LineWidth', 3);
    text(58.45,0.4,['React'],'HorizontalAlignment','right', 'Fontsize', 25);
    annotation('doublearrow', [0.54 0.65], [0.25 0.25], 'LineWidth', 3);
    text(59.25,0.4,['Prepare'],'HorizontalAlignment','right', 'Fontsize', 25);
    annotation('doublearrow', [0.67 0.75], [0.25 0.25], 'LineWidth', 3);
    text(59.95,0.4,['React'],'HorizontalAlignment','right', 'Fontsize', 25);
end
xlabel('time (s)')
